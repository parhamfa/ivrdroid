...
class SessionAuditAcceptanceInstrumentation : Instrumentation() {
    ...
    override fun onStart() {
        ...
        try {
            ...
            val raceCall = PendingCallEvent(raceId, "2026-09-19T10:00:00Z", null, "IVR_HANDLED", null,
                emptyList(), "IN_PROGRESS", 0)
            SecureControlStore.enqueueCall(isolated, raceCall)
            val inFlight = SecureControlStore.pendingCalls(isolated)
            SecureControlStore.acknowledgeCalls(isolated, inFlight, setOf(raceId))
            check(SecureControlStore.pendingCalls(isolated).single().result == "IN_PROGRESS")
            SecureControlStore.enqueueCall(isolated, raceCall.copy(result = "REMOTE_HANGUP", durationSeconds = 10))
            SecureControlStore.acknowledgeCalls(isolated, inFlight, setOf(raceId))
            check(SecureControlStore.pendingCalls(isolated).single().result == "REMOTE_HANGUP")
            val terminal = SecureControlStore.pendingCalls(isolated)
            val first = PendingCallSubEvent("2026-09-19T10:00:05Z", "DIALING", raceId, "-")
            val last = first.copy(occurredAt = "2026-09-19T10:00:10Z", status = "COMPLETED", reason = "CALLER_HANGUP")
            SecureControlStore.appendCallEvent(isolated, raceId, first)
            val submittedEvents = SecureControlStore.pendingCallEvents(isolated)
            SecureControlStore.acknowledgeCalls(isolated, terminal, setOf(raceId))
            SecureControlStore.appendCallEvent(isolated, raceId, last)
            SecureControlStore.acknowledgeCallEvents(isolated, submittedEvents, setOf(raceId))
            check(SecureControlStore.pendingCalls(isolated).isEmpty())
            check(SecureControlStore.pendingCallEvents(isolated)[raceId] == listOf(last))
            SecureControlStore.acknowledgeCallEvents(isolated, mapOf(raceId to listOf(last)), setOf(raceId))
            output.putString("snapshot_ack", "PASS: in-flight parent/event acknowledgements preserve newer and active state")
            ...
            output.putString("during_call", "PASS: encryption yields to calls and resumes from preserved source when idle")
            ...
            SessionAuditSpool.reconcile(isolated)
            check(SecureControlStore.pendingCalls(isolated).single().result == "IN_PROGRESS")
            // Recording reconciliation must never invent a disconnect time or outcome.
            SecureControlStore.enqueueCall(isolated, event.copy(result = "REMOTE_HANGUP", durationSeconds = 16,
                endedAt = "2026-09-12T12:00:16Z", cleanupStatus = "complete"))
            SessionAuditSpool.reconcile(isolated)
            val recovered = SecureControlStore.pendingCalls(isolated).single()
            check(recovered.result == "REMOTE_HANGUP" && recovered.durationSeconds == 16)
            ...
            CallRuntimeState.setBusy(false)
            SessionAuditSpool.reconcile(isolated)
            check(File(rejected, "abort").exists())
