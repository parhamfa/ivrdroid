...
PROMPT_BARGE_IN_COMPILER_VERSION = "4.1.0"
MAXIMUM_SESSION_MS = 600_000
BEEP_DURATION_MS = 500
EXTERNAL_CONTROL_WATCHDOGS_MS = 30_000
...
def compile_flow_v4(
    ...
    """Compile a V4 owned tree into the deterministic signed runtime tape.

    Conversation time after a verified external-call merge is deliberately not part of the
    bounded IVR-control cost. The answer deadline and every continuation are bounded here; the
    call controller owns the connected conversation lifetime.
    """
    ...
    def collect(block, owner_menu_id: str | None, prompt_available: bool = False) -> None:
        ...
        if isinstance(block, PlayPromptBlockV2):
            ...
            collect(block.next, owner_menu_id, prompt_available or bool(block.prompt_id))
            ...
            for branch in sorted(block.branches, key=lambda item: DTMF_ORDER.index(item.digit)):
                ...
                collect(branch.root, block.block_id, branch_prompt_available)
            for name, child in (
                ...
                collect(child, block.block_id, branch_prompt_available)
            ...
            for name, child in (
                ...
                if child is None:
                    raise ValueError(f"{name} has no first step.")
                collect(child, owner_menu_id, prompt_available)
            ...
            collect(block.next, owner_menu_id, prompt_available)
            collect(block.on_unavailable, owner_menu_id, prompt_available)
            ...
            for name, child in (
                ...
                if child is None:
                    raise ValueError(f"External call {name.lower()} path has no first step.")
                collect(child, owner_menu_id, prompt_available)
    ...
    compiler_version = (
        PROMPT_BARGE_IN_COMPILER_VERSION
        if any(
            isinstance(block, CollectDigitBlockV2) and block.allow_prompt_barge_in
    ...
    for index, (block, owner_menu_id) in enumerate(entries):
        ...
        if isinstance(block, PlayPromptBlockV2):
            ...
            if compiler_version == PROMPT_BARGE_IN_COMPILER_VERSION:
                instruction["allow_prompt_barge_in"] = block.allow_prompt_barge_in
        elif isinstance(block, ScheduleBranchBlockV2):
            instruction.update(
            ...
            instruction.update(
                ...
                answer_timeout_ms=block.answer_timeout_seconds * 1000,
                next_pc=pc(block.next),
                on_not_connected_pc=pc(block.on_not_connected),
                on_system_failure_pc=pc(block.on_system_failure),
            )
        elif isinstance(block, ReturnToMenuBlockV2):
    ...
    def maximum_cost(pc_value: int, returns: tuple[int, ...]) -> int:
        ...
        if state in active:
            raise ValueError("V4 program has an unbounded runtime cycle.")
        active.add(state)
        try:
            ...
            if operation == "collect_digit":
                ...
                targets.extend([instruction["on_timeout_pc"], instruction["on_invalid_pc"]])
            ...
            if operation == "external_call":
                continuation = max(
                    maximum_cost(instruction[key], returns)
                    for key in ("next_pc", "on_not_connected_pc", "on_system_failure_pc")
                )
                return (
                    EXTERNAL_CONTROL_WATCHDOGS_MS
                    + instruction["answer_timeout_ms"]
                    + continuation
                )
            ...
            raise ValueError(f"Unsupported V4 operation: {operation}")
        finally:
            active.remove(state)
    ...
    program = {
        "version": 4,
        "entry_pc": 0,
        "maximum_automated_session_ms": maximum_session_ms,
