...
RELEASE_DIR=${1:-"$REPO_DIR/../ivrdroid-release-artifacts/0.9.0/$IVRDROID_SOURCE_COMMIT"}
mkdir -p "$RELEASE_DIR"
...
cp app/build/outputs/apk/debug/app-debug.apk "$RELEASE_DIR/IVRdroid-0.9.0.apk"
cp app/build/outputs/apk/androidTest/debug/app-debug-androidTest.apk "$RELEASE_DIR/IVRdroid-0.9.0-acceptance.apk"
cp app/dist/IVRdroid-system-app-0.9.0.zip "$RELEASE_DIR/"
cp helper/dist/IVRdroid-helper-0.9.0-disabled.zip "$RELEASE_DIR/"
